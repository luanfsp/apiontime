...
class BarberController extends Controller
{
    ...
    public function createRandom() {
        ...
        for($q=0; $q<15; $q++) {
            $names = ['Luan', 'Marcelo', 'Thiago', 'Alecir', 'Leticia', 'Gabriel', 'Gabriela', 'Thais', 'Luiz', 'Diogo', 'José', 'Jeremias', 'Francisco', 'Dirce', 'Marcelo' ];
    ...
    */
    ...
    public function setAppointment($id, Request $request) {
        ...
        if($barberservice) {
            ...
            if(strtotime($apDate) > 0) {
                //3. verificar se o barbeiro já possui agendamento neste dia/horário
                $apps = UserAppointment::select()
                    ->where('id_barber', $id)
                    ->where('ap_datetime', $apDate)
                ->count();
                if($apps === 0) {
                    //4.1 verificar se atendimento nesta data
                    ...
                    ->first();
                    if($avail) {
                        //4.2 verificar se atende nesta hora
                        ...
                        if(in_array($hour.':00', $hours)) {
                            //5. fazer o agendamento
                             ...
                             $array['error'] = 'Serviço indisponível para esta hora';
                        }
                    } else {
                        $array['error'] = 'Serviço indisponível para este dia';
                    }
                } else {
    ...
    public function search(Request $request) {
        ...
        if($q) {
            ...
            $array['error'] = 'Digite para buscar.';
        }
